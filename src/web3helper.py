...
class _Web3Helper(IWeb3Helper):
    ...
    def send_transaction(self, transaction_info, private_key=''):
        if private_key:
            self.set_private_key(private_key)
        _web3 = self.web3
        transaction = self.prepare_transaction(transaction_info)
        ...
        print(f'sending transaction from {self.public_key} :\nInput data : {transaction}')
